...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.coinTask.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.coin-tasks.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('coin_earn') ? 'is-invalid' : '' }}" type="number" name="coin_earn" id="coin_earn" value="{{ old('coin_earn', '0') }}" step="1">
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\CoinTask::SINGLE_TASK_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('single_task') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="single_task_{{ $key }}" name="single_task" value="{{ $key }}" {{ old('single_task', '0') === (string) $key ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('task_finished_bies') ? 'is-invalid' : '' }}" name="task_finished_bies[]" id="task_finished_bies" multiple>
                    @foreach($task_finished_bies as $id => $task_finished_by)
                        <option value="{{ $id }}" {{ in_array($id, old('task_finished_bies', [])) ? 'selected' : '' }}>{{ $task_finished_by }}</option>
                    @endforeach
                </select>
