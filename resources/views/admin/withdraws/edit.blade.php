    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.withdraws.update", [$withdraw->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('coin_amount') ? 'is-invalid' : '' }}" type="number" name="coin_amount" id="coin_amount" value="{{ old('coin_amount', $withdraw->coin_amount) }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('rate') ? 'is-invalid' : '' }}" type="number" name="rate" id="rate" value="{{ old('rate', $withdraw->rate) }}" step="0.01">
                @if($errors->has('rate'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('inr_amount') ? 'is-invalid' : '' }}" type="number" name="inr_amount" id="inr_amount" value="{{ old('inr_amount', $withdraw->inr_amount) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('approved_by') ? 'is-invalid' : '' }}" name="approved_by_id" id="approved_by_id">
                    @foreach($approved_bies as $id => $approved_by)
                        <option value="{{ $id }}" {{ (old('approved_by_id') ? old('approved_by_id') : $withdraw->approved_by->id ?? '') == $id ? 'selected' : '' }}>{{ $approved_by }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Withdraw::STATUS_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('status') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="status_{{ $key }}" name="status" value="{{ $key }}" {{ old('status', $withdraw->status) === (string) $key ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('transaction') ? 'is-invalid' : '' }}" type="text" name="transaction" id="transaction" value="{{ old('transaction', $withdraw->transaction) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('transaction_data') ? 'is-invalid' : '' }}" type="text" name="transaction_data" id="transaction_data" value="{{ old('transaction_data', $withdraw->transaction_data) }}">
