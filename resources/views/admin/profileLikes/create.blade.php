...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.profileLike.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.profile-likes.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('profile_by') ? 'is-invalid' : '' }}" name="profile_by_id" id="profile_by_id">
                    @foreach($profile_bies as $id => $profile_by)
                        <option value="{{ $id }}" {{ old('profile_by_id') == $id ? 'selected' : '' }}>{{ $profile_by }}</option>
                    @endforeach
                </select>
