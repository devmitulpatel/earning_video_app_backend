...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.follower.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.followers.update", [$follower->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('host') ? 'is-invalid' : '' }}" name="host_id" id="host_id">
                    @foreach($hosts as $id => $host)
                        <option value="{{ $id }}" {{ (old('host_id') ? old('host_id') : $follower->host->id ?? '') == $id ? 'selected' : '' }}>{{ $host }}</option>
                    @endforeach
                </select>
                @if($errors->has('host'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('followers') ? 'is-invalid' : '' }}" name="followers[]" id="followers" multiple>
                    @foreach($followers as $id => $followers)
                        <option value="{{ $id }}" {{ (in_array($id, old('followers', [])) || $follower->followers->contains($id)) ? 'selected' : '' }}>{{ $followers }}</option>
                    @endforeach
                </select>
