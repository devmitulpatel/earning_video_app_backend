...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.videoLike.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.video-likes.update", [$videoLike->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('video') ? 'is-invalid' : '' }}" name="video_id" id="video_id">
                    @foreach($videos as $id => $video)
                        <option value="{{ $id }}" {{ (old('video_id') ? old('video_id') : $videoLike->video->id ?? '') == $id ? 'selected' : '' }}>{{ $video }}</option>
                    @endforeach
                </select>
                @if($errors->has('video'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('like_bies') ? 'is-invalid' : '' }}" name="like_bies[]" id="like_bies" multiple>
                    @foreach($like_bies as $id => $like_by)
                        <option value="{{ $id }}" {{ (in_array($id, old('like_bies', [])) || $videoLike->like_bies->contains($id)) ? 'selected' : '' }}>{{ $like_by }}</option>
                    @endforeach
                </select>
