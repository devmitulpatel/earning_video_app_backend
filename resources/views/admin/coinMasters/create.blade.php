...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.coinMaster.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.coin-masters.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('coin_amount') ? 'is-invalid' : '' }}" type="number" name="coin_amount" id="coin_amount" value="{{ old('coin_amount', '0') }}" step="1">
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\CoinMaster::TYPE_TRANSACTION_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('type_transaction') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="type_transaction_{{ $key }}" name="type_transaction" value="{{ $key }}" {{ old('type_transaction', '0') === (string) $key ? 'checked' : '' }}>
